...
path+=('/home/met/.cargo/bin')
path+=('/home/met/.local/bin')
export PATH
...
alias ld='lsd -ald --color=always --group-directories-first .*'
alias lt='lsd --tree'

alias dir='lsd'
alias walk='walk --icons --preview'
alias walkd='walk --icons --preview --dir-only'
...
alias ytdl="yt-dlp -x --embed-metadata --embed-thumbnail --convert-thumbnail jpg --exec-before-download \"ffmpeg -i %(thumbnails.-1.filepath)q -vf crop=\\"'if(gt(ih,iw),iw,ih)':'if(gt(iw,ih),ih,iw)'\\" _%(thumbnails.-1.filepath)q\" --exec-before-download \"rm %(thumbnails.-1.filepath)q\" --exec-before-download \"mv _%(thumbnails.-1.filepath)q %(thumbnails.-1.filepath)q\" --output \"%(title)s.%(ext)s\""
